...
#ifdef HAVE_STB_IMAGE
    // Write PNG using stb_image_write and verify loading via stb
    std::string pngname = "temp_test_tex.png";
    int stride = w * 4;
    if (stbi_write_png(pngname.c_str(), w, h, 4, data, stride)) {
        auto pngtex = Genesis::Engine::Texture::CreateFromFile(pngname);
        REQUIRE(pngtex != nullptr);
        Genesis::Engine::TextureRegistry::Instance().UploadAllToRenderer(cur);
        REQUIRE(pngtex->GetID() != 0);
        remove(pngname.c_str());
    } else {
        WARN("stbi_write_png failed to write PNG file");
    }
#endif
