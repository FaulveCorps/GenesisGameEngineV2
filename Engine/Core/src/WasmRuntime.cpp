...
// Forward declarations for host imports
...
static M3Result LinkHostFunctions(IM3Module module) {
    ...
    // engine_create_body: i32 create_body(i32 mass_fixed, i32 x_fixed, i32 y_fixed, i32 sx_fixed, i32 sy_fixed)
        M3Result r = m3_LinkRawFunction(module, "env", "engine_create_body", "i(iiiii)", (M3RawCall)engine_create_body);
    if (r) return r;
    // engine_destroy_body: void destroy_body(i32 handle)
        r = m3_LinkRawFunction(module, "env", "engine_destroy_body", "v(i)", (M3RawCall)engine_destroy_body);
    if (r) return r;
    // engine_apply_impulse: void apply_impulse(i32 handle, i32 ix_fixed, i32 iy_fixed)
        r = m3_LinkRawFunction(module, "env", "engine_apply_impulse", "v(iii)", (M3RawCall)engine_apply_impulse);
    if (r) return r;
    // engine_create_distance_joint: i32 create_distance_joint(i32 a, i32 b, i32 ax_fixed, i32 ay_fixed, i32 bx_fixed, i32 by_fixed)
        r = m3_LinkRawFunction(module, "env", "engine_create_distance_joint", "i(iiiiii)", (M3RawCall)engine_create_distance_joint);
    return r;
}
