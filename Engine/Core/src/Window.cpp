...
bool Window::Init(const std::string& title, int width, int height) {
    ...
    // Enable SDL logging for diagnostics (SDL3)
    ...
    bool events_result = SDL_Init(SDL_INIT_EVENTS);
    const char* events_error = SDL_GetError();
    
    std::cout << "SDL_Init(EVENTS) returned: " << (events_result ? "true" : "false") << std::endl;
    ...
    if (!events_result) {
        ...
        std::cerr << "This indicates a corrupted SDL3.dll or system incompatibility" << std::endl;
        std::cerr << "\nPossible solutions:" << std::endl;
        std::cerr << "1. Rebuild vcpkg SDL3: vcpkg remove sdl3 && vcpkg install sdl3" << std::endl;
    ...
    std::cout << "SDL_Init(VIDEO) returned: " << (init_result ? "true" : "false") << std::endl;
    ...
    // SDL3: x/y arguments removed (use properties or center by default)
